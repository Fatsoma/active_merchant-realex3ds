...
class RemoteRealexTest < Test::Unit::TestCase
  ...
  def test_invalid_credit_card_name
    ...
    assert_equal(
      'Mandatory field not present - cannot continue. Please check the Developer Documentation for mandatory fields',
      response.message
    )
  end
  ...
  def test_realex_purchase_then_void
    ...
    void_response = @gateway.void(
      purchase_response.authorization,
      order_id: order_id,
  ...
  def test_realex_purchase_then_credit
    ...
    @gateway_with_refund_password = Realex3dsGateway.new(
      fixtures(:realex_with_account).merge(rebate_secret: 'refund')
    )
    ...
    rebate_response = @gateway_with_refund_password.credit(
      @amount,
      purchase_response.authorization,
      order_id: order_id,
      pasref: purchase_response.params['pasref']
