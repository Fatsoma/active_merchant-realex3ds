...
module ActiveMerchant
  module Billing
    ...
    class Realex3dsGateway < Gateway
      URL = 'https://epage.payandshop.com/epage-remote.cgi'.freeze
      THREE_D_SECURE_URL = 'https://epage.payandshop.com/epage-3dsecure.cgi'.freeze
      RECURRING_PAYMENTS_URL = 'https://epage.payandshop.com/epage-remote-plugins.cgi'.freeze
      ...
      BANK_ERROR = 'Gateway is in maintenance. Please try again later.'.freeze
      REALEX_ERROR = BANK_ERROR
      ERROR = 'Gateway Error'.freeze
      CLIENT_DEACTIVATED = ERROR

      def initialize(options = {})
        requires!(options, :login, :password)
        options[:refund_hash] = Digest::SHA1.hexdigest(options[:rebate_secret]) if options.key?(:rebate_secret)
        @options = options
        super
      end
      ...
      def build_3d_secure_verify_signature_or_enrolled_request(action, money, credit_card, options)
        ...
        xml.tag! 'request', 'timestamp' => timestamp, 'type' => action do
          ...
          if action == '3ds-verifysig' && options[:three_d_secure_auth]
            xml.tag!('pares', options[:three_d_secure_auth][:pa_res])
          end
      ...
      def build_receipt_in_request(money, _credit_card, options)
        timestamp = self.class.timestamp
        xml = Builder::XmlMarkup.new(indent: 2)
